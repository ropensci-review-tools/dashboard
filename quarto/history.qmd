...
```{ojs}
minYear = Math.min(...submissions.map(d => d.year));
maxYear = Math.max(...submissions.map(d => d.year));
// `value` controls initial value; default = (min + max) / 2
viewof year = Inputs.range([minYear, maxYear], { step: 1, label: "Start year" })
```

```{ojs}
submissionsFiltered = {
    return submissions.filter(d => d.year >= year);
}
```
