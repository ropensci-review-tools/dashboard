...
// `value` controls initial value; default = (min + max) / 2
viewof submissionYear = Inputs.range([minYear, maxYear], { step: 1, label: "Start year" })
```
...
viewof reviewYear = Inputs.range([minYear, maxYear], { step: 1, label: "Start year" })
```
...
```{ojs}
viewof revDurYear = Inputs.range([minYear, maxYear], { step: 1, label: "Start year" })
```
...
```{ojs}
viewof numEdsYear = Inputs.range([minYear, maxYear], { step: 1, label: "Start year" })
```
