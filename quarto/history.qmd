...
submissionsFilteredAll = {
    return submissionsFiltered.filter(d => d.type === "all");
}
...
q <- dashboard:::gh_editors_team_qry (stats = FALSE)
gh_editors <- gh::gh_gql (query = q)
gh_editors <- gh_editors$data$organization$team$members$nodes
gh_editors <- vapply (gh_editors, function (i) i$login, character (1L))
eds_to_rm <- dashboard:::eds_to_remove
guest_editors <- dashboard:::guest_editors
index <- grep ("\\-", guest_editors)
if (length (index) > 0L) {
    guest_editors <- c (guest_editors, gsub ("\\-", ".", guest_editors [index]))
}
gh_editors <- gh_editors [which (!gh_editors %in% c (eds_to_rm, guest_editors))]
# The GH API delivers some results with names modified from "a-a" to "a.a":
index <- grep ("\\-", gh_editors)
gh_editors <- c (gh_editors, gsub ("\\-", ".", gh_editors [index]))
```

```{r clean-editor-status-history, echo = FALSE}
# Taken straight from editors.qmd
#| cache: false
ed_status <- dplyr::filter (ed_status, !editor %in% c (eds_to_rm, guest_editors)) |>
    ...
    dplyr::filter (editor %in% gh_editors)
```
...
ed_dat$timeline_total <- dplyr::filter (
    ed_dat$timeline_total,
    !name %in% c (eds_to_rm, guest_editors)
)
...
ed_dates$end [ed_dates$name %in% gh_editors] <- max (ed_dates$end)
