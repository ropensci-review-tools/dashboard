...
min_reviews <- 5L # Don't show any data for reviews <= this value
start_time <- lubridate::ymd ("2015-01-01")
timeline <- dplyr::filter (ed_timeline_total, month >= start_time) |>
    dplyr::filter (!name %in% c ("haozhu233")) |>
    dplyr::rename (reviews = value) |>
    dplyr::filter (reviews > min_reviews)

ncols <- diff (range (timeline$reviews)) + 1L
cols <- rev (heat.colors (ncols))
timeline$col <- cols [timeline$reviews - min_reviews]
max_reviews <- max (timeline$reviews)
```

The following two graphs illustrate the previous workload of rOpenSci editors.
No editor should concurrently handle more than `r min_reviews` reviews. This
graph therefore only shows data for cases where individual editorial loads have
exceeded that number, with colors scaled from very light yellow for just over
`r min_reviews` concurrent reviews, up to red for the maximum of
`r max_reviews` concurrent reviews.
...
ggplot (timeline, aes (x = month, y = reviews, fill = col)) +
    geom_col (width = 70) +
