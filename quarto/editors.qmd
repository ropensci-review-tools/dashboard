...
msg <- paste0 (
    "The following two tables are the primary indicators of current ",
    "availability of editors. Values in the `last_active` column indicate ",
    "the length of time since last editorial activity, with values ",
...
```{ojs}
filtered = ed_status.filter(function(editor) {
  return status.includes(editor.status) &&
         stats.includes(editor.stats);
})
columns = ["editor", "last_active", "Languages", "Expertise", "status", "stats"]
```
```{ojs}
Inputs.table(filtered, {columns: columns}, {sort: "inactive_days", reverse: true})
```
