...
The "Stats" selector can be used to select statistical software ("true") or
general ("false") editors (or both). Editors with a status of "FREE" are
highlighted yellow in the table, which by default only shows "FREE" editors.
The 'Languages' column only describes whether
editors are capable of handling submissions in any languages other than English
...
```
```{r has-no-free-txt, echo = FALSE, results = "asis", eval = FALSE}
cat (msg)
```
```{r has-free-txt, echo = FALSE, results = "asis", eval = FALSE}
cat (paste0 (
    msg,
...
ed_status_filtered <-
    dplyr::select (ed_status, editor, inactive_for, Languages, Expertise, status, stats, inactive_days) |>
...
function colorTable(data, { colorColumn, colorOpacity = 1, editorColumn, columns = undefined, format = {}, ...options}) {
  const [row] = data;
...
// Inputs.table(
colorTable(filtered,
  {colorColumn: "rowColor", editorColumn: "editor", columns: columns},
  {sort: "inactive_days", reverse: true}
)
```
