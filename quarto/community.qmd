...
Red dots <span style="color:#e15759;font-size:1.1em">●</span> indicate statistical software packages/reviews.

```{ojs}
viewof nEntries = Inputs.range([10, submissions.length], {value: 40, step: 1, label: "Number of entries"})
```
...
```{ojs}
viewof showStats = Inputs.toggle({label: "Stats only", value: false})
```
