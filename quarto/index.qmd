...
if (httr2::resp_status (resp) != 200L) {
    cat ("GH Rest API Request failed with error ", httr2::resp_status (resp), "\n\n")
}
body <- httr2::resp_body_json (resp)$workflow_runs
if (length (body) == 0L) {
    cat ("GH Rest API Request delivered empty body\n\n")
}
...
if (nrow (run_dat) == 0L) {
    cat ("GH Rest API Request delivered no data\n\n")
}
# run_dat <- run_dat [which (run_dat$conclusion == "success"), ]

if (nrow (run_dat) > 0L && "updated_at" %in% names (run_dat)) {
    # i <- which (run_dat$updated_at == max (run_dat$updated_at))
    i <- 1L # Always sorted by latest anyway
