...
dat4 <- dat [grep ("^4", dat$stage), keep, drop = FALSE]
dat4 <- dplyr::rename (dat4, "inactive_for" = elapsed) |>
    dplyr::arrange (dplyr::desc (elapsed_days))
dat4$editor [which (!nzchar (dat4$editor))] <- "(None)"
...
dat5 <- dat [grep ("^5", dat$stage), keep, drop = FALSE]
dat5 <- dplyr::rename (dat5, "inactive_for" = elapsed) |>
    dplyr::arrange (dplyr::desc (elapsed_days))
dat5$editor [which (!nzchar (dat5$editor))] <- "(None)"
